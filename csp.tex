...
When $\mathsf{V}$ is $\mathsf{Set}$, the variety of sets, $\Clo(\mathsf{Set})$ is the clone of projections $\mathbf{N}$, that we denote by $\mathbf{N}$. 
...
\begin{corollary}
    ...
    \begin{proof}
        ...
        Since $\mathbf{A}$ is finite, then $HSP(\mathbf{A})$ is locally finite, 
