...
\begin{lemma}
    ...
    Let $\mathbf{A}=(A, \Phi)$ be an algebra. \textcolor{red}{such that $\Clo_1(\mathbf{A})$ is a group -- where do we need it?} 
    Then the relation 
    \begin{equation}
        \sigma:=\{(\psi(a_1), \ldots, \psi_(a_k)) : \psi \in \Clo_1(\mathbf{A})\}
    \end{equation}
    belongs to $\Inv(\Phi)$. 
    \begin{proof}
